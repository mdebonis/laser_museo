/*  versione 9: implementazione movimento di due servomotori
...
String formCiclo2 = "<br><br>nuovo angolo Y<input type=\"text\" name=\"angY\"><br>nuovo angolo X<input type=\"text\" name=\"angX\"><br>deltaT<input type=\"text\" name=\"delt\"><br>laser acceso<input type=\"radio\" name=\"las\" value=\"lasOn\"><br>acquisisci in file<input type=\"radio\" name=\"fil\" value=\"inFile\"><br><input type=\"submit\" value=\"CONTINUA\"></H2></form> <form method=\"POST\" action= \"/eliminaFile\"><br><input type=\"submit\" value=\"ELIMINA FILE POSIZIONI\"></form>";
...
//CREAZIONE PAGINE DEL WEB SERVER
 ...
 //acquisisce i dati inviati dal form su browser
  server.on("/rispFORMservo", HTTP_POST, [](AsyncWebServerRequest *request)
  {
    ...
    String angoloStrY, angoloStrX, deltaT, laserOn = "0";
    ...
    for(int i=0;i<paramsNr;i++)
    {
        ...
        if (p->name()=="fil")
           {if (p->value()=="inFile")
              { Serial.println(angoloStrX); Serial.println(angoloStrY); Serial.println(laserOn);
    ...
    FORMciclo = formCiclo1 + "x=" + angoloStrX + ", y=" + angoloStrY + formCiclo2;

    request->send(200, "text/html", FORMciclo);

    //esegue il movimento in base ai parametri inseriti nel form
    if(angoloX != -1 && angoloY != -1)
    {
      digitalWrite(laser, LOW);
      servoY_2.write(angoloX);
      servoX_2.write(angoloY);
      delay(500);
  ...
  for (;;)      //loop
  {
     ...
     while (i < len)              //ciclo che legge tutto il file
      { riga = "";
       ...
       int posizione = 0;
       String sTempo = "", sPosX = "", sPosY = "", sMoving = "";
